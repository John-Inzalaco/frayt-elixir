defmodule FraytElixir.CustomContracts.RugDoctorTest do
  use FraytElixir.DataCase

  alias Ecto.Changeset
  alias FraytElixir.CustomContracts.RugDoctor
  ...
  describe "calculate_pricing" do
    ...
    test "with proper parameters" do
      ...
      assert %Changeset{valid?: true} =
               changeset =
               match
               |> RugDoctor.calculate_pricing()

      assert %{
               driver_cut: 0.75,
               fees: [
                 %MatchFee{type: :base_fee, amount: 2200, driver_amount: 1556}
               ],
               match_stops: [
                 %{base_price: 2200, tip_price: 0, driver_cut: 0.75}
               ]
             } =
               changeset
